...
/// Defining the middleware function for authenticating admin users
const adminAuthenticate = async function(req, res, next) {
    try {
        ...
        // Finding the admin user in the database based on the ID stored in the JWT token
        const rootUser = await AdminDb.findById(verifyToken._id);
        
        // Checking if the admin user is not found in the database
        if (!rootUser) {
            throw new Error("User not found");
        }

        // Storing the retrieved admin user, user ID, and token in the request object
